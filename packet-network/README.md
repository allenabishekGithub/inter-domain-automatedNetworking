...
The planned [Containerlab Packet MCP server](../docs/mcp-server-design.md)
wraps this package for domain-agent access. It has separate Packet A and Packet B
instances with independent identities, credentials and resource scopes. The
server is not implemented; the commands below remain the current lab interface.
